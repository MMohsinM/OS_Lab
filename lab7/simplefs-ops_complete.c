...
int simplefs_read(int file_handle, char *buf, int nbytes){
	...
	*/
    ...
    int block_num; //int i = 0,j = BLOCKSIZE;
    if (current_offset + nbytes < MAX_FILE_SIZE * BLOCKSIZE){
    
    		block_num = my_inode.direct_blocks[current_offset /BLOCKSIZE];
    		simplefs_readDataBlock(block_num, buf);
...
int simplefs_write(int file_handle, char *buf, int nbytes){
	...
	*/
    ...
    if (current_offset + nbytes < MAX_FILE_SIZE * BLOCKSIZE){
    	...
    	my_inode.direct_blocks[current_offset / BLOCKSIZE] = block_num;
    	simplefs_writeDataBlock(block_num, buf);
